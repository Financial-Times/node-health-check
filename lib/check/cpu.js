...
module.exports = class CpuCheck extends Check {
	...
	static validateOptions(options) {
		...
		if (!isNumber(options.threshold) || options.threshold < 1 || options.threshold > 200) {
			return new TypeError('Invalid option: threshold must be a number between 1 and 200');
		}
		return true;
	}
